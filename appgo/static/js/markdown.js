    ...
    window.loadDocument = function($node) {
        ...
        $.get(window.editURL + $node.node.id ).done(function (res) {
            ...
            if(res.code === 0){
                ...
                var node = { "id" : res.data.documentId,'parent' : res.data.parentId === 0 ? '#' : res.data.parentId ,"text" : res.data.documentName,"identify" : res.data.identify,"version" : res.data.version};
                pushDocumentCategory(node);
                window.selectNode = node;
                pushVueLists(res.data.attach);
                // 设置 history
                // 20200201 saber 因为后端都加了api作为前缀，来标明是接口
                // 所以页面url，需要去除api
                url = window.editURI
                url = url.replace('/api/web', '');
                if(!window.onpop) history.pushState(node,res.data.doc_name,url + res.data.identify);
                window.onpop=false;
            }else{
                layer.msg("文档加载失败");
            }
