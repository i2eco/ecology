...
function load_doc(url,wd,without_history) {
    NProgress.start();
    $.get(url+"?fr=ecology",function (res) {
        if(res.code === 0){
            var body = res.data.body;
            var docTitle = res.data.docTitle;
            var title = res.data.title;
            var $body = body;
            $("#page-content").html($body);
            // RenderByMarkdown($body);
            $("title").text(title);
            $("#article-title").text(docTitle);

            $(".bookmark-action").attr("data-docid",res.data.docId);
            $(".btn-edit").attr("href",$(".btn-edit").attr("data-url"));
...
function change_url_state(url,title){
    // 20200201 saber 因为后端都加了/api/web作为前缀，来标明是接口
    // 所以页面url，需要去除api
    url = url.replace('/api/web', '');
...
$(function () {
    ...
    $(".bookmark-action").click(function (e) {
        ...
        $.get(href,function (res) {
            if(res.code==0){
                alertTips("success",res.msg,3000,"");
            }else{
                alertTips("danger",res.msg,3000,"");
            }
            if(res.data){//新增书签成功
        ...
        $.post(form.attr("action"),{"keyword":wd},function (res) {
            ...
            if(res.code === 0){
                for(var i in res.data){
                    var item = res.data[i];
                    html += '<li><a data-wd="'+res.msg+'" href="javascript:;" title="'+ item.doc_name +'" data-id="'+ item.identify+'"> '+ item.doc_name +' </a></li>';
                }
            }
