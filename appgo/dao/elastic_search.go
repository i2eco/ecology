...
func NewElasticSearchClient() (client *elasticSearchClient) {
	client = &elasticSearchClient{
		Host:    Global.GetOptionValue("ELASTICSEARCH_HOST", "http://localhost:9200/"),
		Index:   "bookstack",
		Type:    "fulltext",
		On:      Global.GetOptionValue("ELASTICSEARCH_ON", "false") == "true",
...
//创建索引
func (this *elasticSearchClient) BuildIndex(es mysql.ElasticSearchData) (err error) {
	var (
		js []byte
	)
	if !this.On {
...
func (this *elasticSearchClient) Search(wd string, p, listRows int, isSearchDoc bool, bookId ...int) (result ElasticSearchResult, err error) {
	...
	percent := Global.GetOptionValue("SEARCH_ACCURACY", "50")
...
func (this *elasticSearchClient) RebuildAllIndex(bookId ...int) {
	...
	for page := 1; page < maxPage; page++ {
		var books []mysql.Book
		qs := mus.Db.Select("book_id,book_name,label,description,privately_owned").Offset((page - 1) * pageSize).Limit(pageSize)

		if bid > 0 {
			qs.Where("book_id = ?", bookId).Find(&books)
		} else {
			qs.Find(&books)
		}
		...
		if len(books) > 0 {
			...
			if err := this.BuildIndexByBuck(data); err != nil {
				...
				mus.Logger.Error("书籍索引创建失败，书籍ID:" + fmt.Sprintf("%v", bookId))
			} else {
				mus.Logger.Info("书籍索引创建成功，书籍ID:" + fmt.Sprintf("%v", bookId))
			}
		} else {
			page = maxPage
		}
	...
	for page := 1; page < maxPage; page++ {
		...
		//qs := mus.Db.Select("document_id,book_id,document_name,release,vcnt").Offset((page - 1) * pageSize).Limit(pageSize).Order("document desc")
		qs := mus.Db.Offset((page - 1) * pageSize).Limit(pageSize).Order("document_id desc")

		if bid > 0 {
			qs.Where("book_id = ?", bid).Find(&docs)
		} else {
			qs.Find(&docs)
		}
		if l := len(docs); l > 0 {
			...
			for i := 0; i < num; i++ {
				...
				if err := this.BuildIndexByBuck(data); err != nil {
					mus.Logger.Error(err.Error())
					mus.Logger.Error("文档索引创建失败，文档ID:" + fmt.Sprintf("%v", docId))
				} else {
					mus.Logger.Info("文档索引创建成功，文档ID:" + fmt.Sprintf("%v", docId))
				}
			}
